...
import { USER_PROFILE_QUERY } from "../api/publicUsers";
...
function UserProfile({ userID, own }: { userID: string; own: boolean }) {
  const user = useQuery<{ getUser?: User; userProfile?: PublicUser }>(own ? GET_USER_QUERY : USER_PROFILE_QUERY, { variables: { id: userID }, skip: !userID });
  ...
  const profile = own ? user.data?.getUser : user.data?.userProfile;
  ...
  const privateProfile = isFullUser(profile) ? profile : null;

  return <main className="page-shell user-profile-page"><section className="profile-hero"><aside className="profile-identity"><div className="profile-avatar"><AvatarImage avatar={profile.avatar} name={displayName} size={300} eager /></div><dl><div><dt>На платформе</dt><dd>с {formatDate(profile.createdAt)}</dd></div><div><dt>Роль</dt><dd>{privateProfile?.isSuperuser ? "Суперпользователь" : profile.isAdmin ? "Администратор" : "Пользователь"}</dd></div><div><dt>О себе</dt><dd>Информация не добавлена</dd></div></dl></aside><div className="profile-main"><header><div><span className="section-kicker">Профиль</span><h1>{displayName}</h1><p>@{profile.username}</p></div>{own && <Link className="header-icon profile-settings-link" to="/profile/settings" aria-label="Настройки профиля"><Settings size={20} /></Link>}</header>{own && privateProfile && <div className="profile-private"><span>{privateProfile.email}</span>{privateProfile.phone && <span>{privateProfile.phone}</span>}</div>}<section className="attempts-section"><div className="section-title"><div><span className="section-kicker">Решения</span><h2>Последние попытки</h2></div><Code2 size={19} /></div>{(answers.loading || code.loading) && attempts.length === 0 ? <div className="content-state"><Spinner label="Загружаем попытки…" /></div> : !own ? <div className="content-state content-state--compact"><UserRound size={25} /><strong>История решений скрыта</strong><p>Публичная история появится после настройки правил доступа.</p></div> : attempts.length === 0 ? <div className="content-state content-state--compact"><Code2 size={25} /><strong>Попыток пока нет</strong><p>Откройте задачу и отправьте первое решение.</p></div> : <div className="attempt-list">{attempts.map((attempt) => <Link className="attempt-row" key={`${attempt.detail}-${attempt.id}`} to={attempt.href}><span className={`verdict-dot${attempt.verdict === "accepted" ? " is-accepted" : attempt.verdict === "queued" ? " is-queued" : " is-wrong"}`} /><div><strong>Задача {attempt.taskId.slice(0, 8)}</strong><p>{attempt.detail}</p></div><div><strong>{verdictLabel(attempt.verdict)}</strong><p>{formatDateTime(attempt.date)}</p></div></Link>)}</div>}</section></div></section></main>;
}

function verdictLabel(value: string) { return value === "accepted" ? "Принято" : value === "queued" ? "В очереди" : value === "completed" ? "Проверено" : "Ошибка"; }
function isFullUser(user: User | PublicUser): user is User { return "email" in user; }
